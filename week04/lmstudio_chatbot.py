...
# Gold Miner Game Section
st.sidebar.markdown("### ⛏️ Snoopy's Gold Miner")
...
with col1:
    if st.button("🎮 Start Game"):
        ...
        # Generate random gold positions
        game_state["mines"] = [
        ...
        random.shuffle(game_state["mines"])
        st.rerun()
...
if game_state["game_active"]:
    ...
    st.sidebar.metric("💰 Score", game_state["score"])
    st.sidebar.metric("❤️ Lives", game_state["lives"])
    st.sidebar.metric("⏱️ Time", f"{game_state['time_left']}s")
    st.sidebar.metric("📈 Level", game_state["level"])
    
    st.sidebar.markdown("#### ⛏️ Mining Area")
    st.sidebar.markdown("Click to mine! 🐕")
    
    # Create mining buttons in a grid
    cols = st.sidebar.columns(3)
    for i, mine in enumerate(game_state["mines"][:9]):  # Show only first 9
        col_idx = i % 3
        with cols[col_idx]:
            # Different emojis for different items
            if mine["type"] == "gold":
                emoji = "💰" if mine["size"] == "large" else "🥇" if mine["size"] == "medium" else "🪙"
            else:
                emoji = "🪨"
            
            if st.button(f"{emoji}", key=f"mine_{i}"):
                if mine["type"] == "gold":
                    points = mine["value"]
                    game_state["score"] += points
                    st.sidebar.success(f"💎 Found gold! +{points} points!")
                    
                    # Check level up
                    if game_state["score"] > game_state["level"] * 500:
                        game_state["level"] += 1
                        game_state["time_left"] += 30  # Bonus time
                        st.sidebar.info(f"🎉 Level {game_state['level']}! +30s bonus time!")
                else:
                    game_state["lives"] -= 1
                    st.sidebar.error("🪨 Hit a rock! -1 life!")
                    
                    if game_state["lives"] <= 0:
                        game_state["game_active"] = False
                        st.sidebar.error("💀 Game Over! Snoopy needs more practice!")
                
                # Remove the mined item and add a new one
                import random
                game_state["mines"][i] = {
    ...
    # Game tips
    st.sidebar.markdown("#### 💡 Game Tips")
    st.sidebar.markdown("""
    - 💰 Large gold = more points
    - 🪨 Rocks cost lives
    - 🎯 Score 500+ to level up
    - ⏱️ Get bonus time each level
    """)
    
else:
    st.sidebar.markdown("#### 🎮 Ready to Mine?")
    st.sidebar.markdown("Help Snoopy find gold treasures!")
    if game_state["score"] > 0:
        st.sidebar.info(f"🏆 Best Score: {game_state['score']}")
